    ...
    try {
      ...
      response.status === 'OK'
        ? toast.update(toastID, {
            render: 'Successfully posted order !',
            type: 'success',
            isLoading: false,
            autoClose: '800',
          })
        : toast.update(toastID, {
            render: 'Failed posting request !',
            type: 'error',
            isLoading: false,
            autoClose: '800',
          });
  ...
  function HandlePostOrder(event) {
    ...
    if (validateQuantity.length === 1 && validateQuantity[0] === '0') {
      ...
      if (f === false) {
        ...
        postDetailsToAPI();
        setTimeout(() => {
          redirectToHome('/complete-orders');
        }, 1000);
      }
    }
  }
        ...
        <div>
          <form onSubmit={HandlePostOrder}>
            <div className='postEle'>
              <label className='postLabel' name='productType' htmlFor=''>
                Item-Type
              </label>
              <select
                name='productType'
            ...
            <div className='postEle'>
              <label className='postLabel' htmlFor=''>
                Items
              </label>
              <input
                className='postInput'
            ...
            <div className='postEle'>
              ...
              <div className=''>
                <input
                  className='postInput quantityInput'
                  type='text'
                  name='quantity'
                  required
                  onChange={onFieldsChange}
