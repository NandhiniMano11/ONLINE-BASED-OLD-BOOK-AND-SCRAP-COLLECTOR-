...
function CurrentOrders() {
  ...
  React.useEffect(() => {
    async function getPastOrdersFromAPI() {
      try {
        const userPastOrders = await axios.post(
          'http://localhost:4000/api/order/completedorders',
          JSON.stringify({
            id: userObject._id,
          }),
          {
        ...
        const userIncompleteOrders = await axios.post(
          'http://localhost:4000/api/order/incompleteorders',
          JSON.stringify({
            id: userObject._id,
          }),
          {
