...
function Orders(props) {
  ...
  async function buyOrder(id) {
    ...
    try {
      await axios.patch(
        'http://localhost:4000/api/order/ordercomplete',
        JSON.stringify({
          id: id,
    ...
    toast.update(buyingToastID, {
      render: 'Successfully placed order !',
      type: 'success',
      isLoading: false,
      autoClose: '800',
    });
    ...
    async function getOrdersFromAPI() {
      try {
        const userOrders = await axios.post(
          'http://localhost:4000/api/order/orderBycity',
          JSON.stringify({
            id: userObject._id,
          }),
          {
    ...
    <div>
      ...
      {orders.length === 0 ? (
        <div className='empty-cart'>
          ...
          <div className='empty-cart-text'>No orders available to buy 🥲 </div>
        </div>
      ) : (
        orders.map((val) => {
          return (
            <div key={val._id} className='order'>
              <div className='order-grid grid-4'>
                ...
                <div className='customer-data'>
                  <h3>
                    {' '}
                    <span className='green'> Ordered On : </span>
                    {val.date.substring(0, 10)}
                  </h3>
                  <h3>
                    {' '}
                    <span className='green'>Phone No:</span>{' '}
                    <span className='shade'>{val.phone}</span>
                  </h3>
                  <h3>
                ...
                <div className='order-details'>
                  ...
                  <h3>
                    <span className='green'>Approx Quantity:</span>
                    {val.quantity + ' Kg'}
                  </h3>
                  <h3>
                    <span className='green'>Items:</span>
                    {val.items}
                  </h3>
                </div>
